...
//						for(int i = 0; i<o.getHighlightFields().get("pdfText").fragments().length; i++)
//							highlightedText += o.getHighlightFields().get("pdfText").fragments()[i];
//						result.setPdfText(highlightedText);
//					}
//						
//
...
//		System.out.println(qb);
//		
//		SearchResponse response = elasticSearchTemplate.getClient().prepareSearch("scientificpaper").setQuery(boolQuery.mustNot(qb)).get();
//		
//		for (SearchHit o : response.getHits()) {
//			ObjectMapper objectMapper = new ObjectMapper();
//			ScientificPaperIndexUnit result = null;
